...
class SelfAttention(nn.Module):
    def __init__(self, embed_size, heads):
        ...
        assert(self.head_dim*heads == embed_size) #embed size needed to be div by heads
    #q,v,k
        self.values = nn.Linear(self.head_dim, self.head_dim, bias = False)
        self.keys = nn.Linear(self.head_dim, self.head_dim, bias = False)
        self.qeuries = nn.Linear(self.head_dim, self.head_dim, bias = False)
    ...
    def forward(self, values, keys, query, mask):
        ...
        keys = self.keys(keys)
        queries= self.qeuries(queries)
        energy =  torch.ensum("nqhd,nkhd->nhqk",[queries, keys])#N, q=query, h = head , d = dim
        #queries shape :(N, query_len, heads_dim)
        #key shape:(N, key_len, heads, heads_dim)
        #now energy shape : (N, heads, query_len, key_len)
...
class DecoderBlock(nn.Module):
    def __int__(self, embed_size, heads, forward_expansion, dropout, device):
        super(DecoderBlock, self).__int__()
        self.attention = SelfAttention(embed_size, heads)
...
class Decoder(nn.Module):
    def __int__(self,trg_vocab_size, embed_size, num_layers, heads, forward_expansion, dropout, device, max_length,):
    ...
    def __forward(self, x, enc_out, src_mask, trg_mask):
        N,seq_length = x.shape
        positions = torch.arange(0, seq_length).expand(N,seq_length).to(self.device)
...
class Transformer(nn.Module):
        ...
        def forward(self, src, trg):
            src_mask = self.make_src_make(src)
